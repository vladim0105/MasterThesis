...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        prob_idx = (top5_catid == scorp_idx).nonzero().item()
        ...
        pred, anom, anomp = htm_layer(htm_in, prob>=0.95)

        anomalies.append(anom)

        dataline = f"{anom:.3f},{prob:.3f},{categories[top5_catid[0]]}"
        data.append(dataline)
        print(f"({counter}) {file} : {dataline}")
        counter += 1
    print(np.corrcoef(np.array(anomalies), np.array(probs)))
    with open("probs.json", "w") as f:
        json.dump({"data": data}, f)
    xs = np.arange(1, len(anomalies) + 1, 1)
    inserted_anoms = [549+1, 500+1, 665+1]
    plt.plot(xs, anomalies, color="blue", label="htm anomalies")
    plt.vlines(inserted_anoms, 0, 1, color="red", label="inserted anomalies", alpha=0.5)
    plt.xlabel("Image #")

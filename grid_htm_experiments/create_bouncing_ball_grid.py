...
def create_video(path, r, gridhtm: model.GridHTM, shape):
    out = cv2.VideoWriter('bb_results/bouncing_ball_grid.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 20,
                          (shape[0], shape[1]), True)
    sp_out = cv2.VideoWriter('bb_results/bouncing_ball_sp_grid.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 20,
    ...
    with progressbar.ProgressBar(max_value=len(path),
                                 widgets=["Processing Frame #", progressbar.SimpleProgress(), " | ",
                                          progressbar.ETA(),"|", progressbar.AdaptiveTransferSpeed(unit="frames")]) as bar:
        for point in path:
            frame = np.zeros(shape).astype(np.uint8)
            frame = fill_circle(point[0], point[1], r, frame)
            bit_frame = (frame == 255)[:, :, 0].astype(np.uint8)
            anom, col_sp_output, raw_anoms = gridhtm(bit_frame)
            anoms.append(anom)

...
\begin{figure}[htb]
    \centering
    \includegraphics[width=0.6\linewidth]{resources/related_works/sp_vis_alt}
...
\par
For high-dimensional anomaly detection; \textcite{MotionAnomalyDetection} used a \gls*{htm} system to find anomalous frames in videos of motions, example shown in \autoref{fig:motion_frame}. The anomalies were artificially created by swapping certain frames between different motion videos in the dataset. The results show that the \gls*{htm} system was able to correctly detect some anomalies, but not an impressive amount.
\begin{figure}[htb]
    \centering
...
\par
With the topics covered in the chapter in mind...
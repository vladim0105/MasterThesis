...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{resources/related_works/perceptron.png}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{Grad-CAM visualization for dog and cat. (Source:~\cite{GradCamExample})}
\end{figure}
...
\par
As previously stated, generative models suffer from a lack of explainability, and since generative models make up most of the state-of-the-art approaches in anomaly detection, it is safe to say that there is a lack of explainability in the field of anomaly detection in videos.
\par
...
\end{itemize}
Current HTM implementations only model L2/L3, but can be extended to model L4 as well~\cite{htm_l2_l3}.
\par
...
\begin{figure}[H]
    ...
    \caption{Comparison of neurons~\cite{htm_neurons}: Traditional Machine Learning (A), Biological Neuron (B), HTM Neuron (C). Source:~\cite{BAMI}}
...
\begin{figure}[H]
    ...
    \caption{Example thresholding on the Fashion MNIST dataset~\cite{mnist_fashion}.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{resources/related_works/resnet18_layer_10.png}
...
\end{itemize}
There are also applications that are used in production, such as the model offered by \href{www.cortical.io}{cortical.io} which builds upon HTM in order to perform language analysis. This is made this possible by introducing Semantic Folding and Semantic Fingerprinting~\cite{semantic_folding}.

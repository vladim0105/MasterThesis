...
The following is relevant background information on Deep Learning, Video Anomaly Detection, and \gls*{htm} theory.
...
\begin{figure}[H]
    ...
    \caption[Gradient Descent Comparison]{Comparison between \gls*{sgd} and gradient descent.}
    \label{fig:gd_sgd}
\end{figure}
\par
...
\end{itemize}
Current \gls*{htm} implementations only model cortical columns, and layer-wise only L2/L3 but can be extended to model L4 as well~\cite{htm_l2_l3}.
\par
...
\par
Seeing as there is currently no encoder that can produce sensible SDRs from high dimensional data such as videos, a natural conclusion to make is that \gls*{htm} should be applied differently, and that a new type of architecture using \gls*{htm} should be explored for this purpose.
\subsection{Learning}
...
\begin{figure}[H]
    ...
    \caption[The HTM Pipeline]{A typical \gls*{htm} pipeline. A common next-step could be to use a classifier to convert the predictive SDR into a classification. }

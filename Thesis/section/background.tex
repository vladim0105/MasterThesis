...
\begin{figure}[H]
    \centering
    \includegraphics[]{resources/related_works/sp_vis.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[]{resources/related_works/tm_vis1.png}
...
\section{Thousand Brains Theory}
...
\end{figure}
While the Thousand Brains Theory is not implemented in any way in a standard HTM model, it does show that recent developments within deep learning for image analysis have similarities with HTM theory. 
...
The current state of the art algorithms are numerous, but the main approach is achieved by using deep learning. Within deep learning, the most popular approach is to use a GAN to generate synthetic data and compare it to real data in order to detect anomalies. This approach is based on the assumption that the model will only be able to generate data similar to what it has been trained on, and therefore fail when an anomalous event occurs. 

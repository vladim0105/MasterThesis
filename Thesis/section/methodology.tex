...
\end{figure}
Since the TM can only grow segments to cells that were active in the previous timestep, it means that it struggles to learn temporal patterns. One example of such a pattern is when a car stands still for a long time. This means that if a car were to suddenly stop in the middle of a road, then the system would not mark it as an anomaly as the pattern is the same as if the car was in motion. The solution is to concatenate the past $n$ SP outputs as input into the TM.
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{example-image-a}
